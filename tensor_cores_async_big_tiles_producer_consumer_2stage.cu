...
__global__ void tensor_core_matmul_async_swizzle_BT_PC2(int n_elem, const half* a, const half* b, half* c)
{
    ...
    if (threadIdx.z == 0)
    {
        auto token1 = bar[0].arrive();
        auto token2 = bar[1].arrive();
    }
    ...
    for (int32_t tile = 0; tile < n_elem; tile+=BK*WMMA_MKN)
    {
        if(threadIdx.z == 1)
        {
            ...
            auto token = bar[2 + stage].arrive();
        }
        if (threadIdx.z == 0)
        {
            bar[2 + stage].arrive_and_wait();
...
template<int BM, int BN, int BK, int OUT_TILES>
double check_configuration_async_BT_PC2(half* a, half*b, half* output, int N)
{
    dim3 dimBlock(1,1,1);
    dim3 dimGrid(1,1,1);
    ...
    cudaFuncSetAttribute(tensor_core_matmul_async_swizzle_BT_PC2<BM, BN, BK, OUT_TILES>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    return measure_performance([&](){ tensor_core_matmul_async_swizzle_BT_PC2<BM, BN, BK, OUT_TILES><<<dimGrid, dimBlock, smem_size>>>(N, a, b, output); });
}
double TensorCoresAsyncBT_PC2Kernel::run(half* a, half* b, half* cublas_ref, int N)
{
    double matmul_time = std::numeric_limits<double>::max();

    matmul_time = std::min(matmul_time, check_configuration_async_BT_PC2<8, 8, 4, 4>(a, b, output, N));
