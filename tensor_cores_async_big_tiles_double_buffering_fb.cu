...
__global__ void tensor_core_matmul_async_swizzle_BT_DB_FB(int n_elem, const half* a, const half* b, half* c)
{
    ...
    for (int32_t tile = 0; tile < n_elem; tile+=BK*WMMA_MKN)
    {
        ...
        {
            unsigned int ld_stage = (stage + 1)%2;
            const half* b_curr = b + (tile)*n_elem + blockIdx.y*BN*WMMA_MKN + gridDim.y * BN*WMMA_MKN;
    ...
    const half* b_curr2 = b + (n_elem - 2*BK*WMMA_MKN)*n_elem + blockIdx.y*BN*WMMA_MKN + gridDim.y * BN*WMMA_MKN;
    ...
    CP_ASYNC_COMMIT_GROUP();
    unsigned int b_stage = (stage + 1)%2;
    for (int k = 0; k<BK; k+=2)
    {
        ...
        for (int n = 0; n < OUT_TILES; n++)
        {
            load_tile_b_shared_swizzle<S_BITS_B>(b_tile[0], b_smem + b_stage*B_ST_STRIDE, (k*BN*WMMA_MKN + laneN*OUT_TILES + n)*WMMA_MKN, BN*WMMA_MKN, lane_id);
            load_tile_b_shared_swizzle<S_BITS_B>(b_tile[1], b_smem + b_stage*B_ST_STRIDE, ((k+1)*BN*WMMA_MKN + laneN*OUT_TILES + n)*WMMA_MKN, BN*WMMA_MKN, lane_id);
            for (int m = 0; m < OUT_TILES; m++)
            {
                mma(a_tile[0][m], b_tile[0], acc[m][n]);
            }
    ...
    for (int32_t tile = n_elem - 2*BK*WMMA_MKN; tile >= 0; tile-=BK*WMMA_MKN)
    {
        unsigned int ld_stage = (stage + 1)%2;
        CP_ASYNC_WAIT_GROUP(0);
        __syncthreads();
...
template<int BM, int BN, int BK, int OUT_TILES>
double check_configuration_async_BT_DB_FB(half* a, half*b, half* output, int N)
{
    dim3 dimBlock(1,1,1);
    dim3 dimGrid(1,1,1);
    ...
    cudaFuncSetAttribute(tensor_core_matmul_async_swizzle_BT_DB_FB<BM, BN, BK, OUT_TILES>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    return measure_performance([&](){ tensor_core_matmul_async_swizzle_BT_DB_FB<BM, BN, BK, OUT_TILES><<<dimGrid, dimBlock, smem_size>>>(N, a, b, output); });
}
double TensorCoresAsyncBT_DB_FBKernel::run(half* a, half* b, half* cublas_ref, int N)
{
    double matmul_time = std::numeric_limits<double>::max();

    matmul_time = std::min(matmul_time, check_configuration_async_BT_DB_FB<8, 8, 2, 4>(a, b, output, N));
    // debug_print(b,  N, true);
    // debug_print(output,  N, true);
    // debug_print(cublas_ref,  N, false);
    test_output(cublas_ref, N);

    matmul_time = std::min(matmul_time, check_configuration_async_BT_DB_FB<16, 8, 2, 4>(a, b, output, N));
    test_output(cublas_ref, N);
