...
__global__ void tensor_core_matmul_async_swizzle_BT_DB_idx(int n_elem, const half* a, const half* b, half* c)
{
    ...
    idx = stage*B_ST_STRIDE + laneN*OUT_TILES*WMMA_MKN + (lane_id%16)*BN*WMMA_MKN + (lane_id/16)*8;
...
template<int BM, int BN, int BK, int OUT_TILES>
double check_configuration_async_BT_DB_idx(half* a, half*b, half* output, int N)
{
    dim3 dimBlock(1,1,1);
    dim3 dimGrid(1,1,1);
    ...
    cudaFuncSetAttribute(tensor_core_matmul_async_swizzle_BT_DB_idx<BM, BN, BK, OUT_TILES>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    return measure_performance([&](){ tensor_core_matmul_async_swizzle_BT_DB_idx<BM, BN, BK, OUT_TILES><<<dimGrid, dimBlock, smem_size>>>(N, a, b, output); });
}
double TensorCoresAsyncBT_DB_IdxKernel::run(half* a, half* b, half* cublas_ref, int N)
{
