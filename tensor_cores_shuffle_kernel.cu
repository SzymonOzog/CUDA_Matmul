...
double check_configuration_shuffle(half* a, half*b, half* output, int N)
{
    dim3 dimBlock(1,1,1);
    dim3 dimGrid(1,1,1);
...
double TensorCoresShuffleKernel::run(half* a, half* b, half* cublas_ref, int N)
{
    ...
    matmul_time = std::min(matmul_time, check_configuration_shuffle<8, 2>(a, b, output, N));
    test_output(cublas_ref, N);
    //
    // matmul_time = std::min(matmul_time, check_configuration_shuffle<9, 3>(a, b, output, N));
    // test_output(cublas_ref, N, 1e-2);
    //
    matmul_time = std::min(matmul_time, check_configuration_shuffle<8, 4>(a, b, output, N));
    test_output(cublas_ref, N);
