...
__global__ __maxnreg__(128) void tensor_core_matmul_async_swizzle_BT_DB_FB_Idx(int n_elem, const half* a, const half* b, half* c)
{
    ...
    for (int32_t tile = n_elem - BK*WMMA_MKN; tile >= 0; tile-=BK*WMMA_MKN)
    {
        ...
        CP_ASYNC_WAIT_GROUP(0);
        __syncthreads();
        if(tile - BK*WMMA_MKN == n_elem -2*BK*WMMA_MKN)
        {
...
template<int BM, int BN, int BK, int OUT_TILES>
double check_configuration_async_BT_DB_FB_Idx(half* a, half*b, half* output, int N)
{
    dim3 dimBlock(1,1,1);
    dim3 dimGrid(1,1,1);
    ...
    cudaFuncSetAttribute(tensor_core_matmul_async_swizzle_BT_DB_FB_Idx<BM, BN, BK, OUT_TILES>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    return measure_performance([&](){ tensor_core_matmul_async_swizzle_BT_DB_FB_Idx<BM, BN, BK, OUT_TILES><<<dimGrid, dimBlock, smem_size>>>(N, a, b, output); });
}
double TensorCoresAsyncBT_DB_FB_IdxKernel::run(half* a, half* b, half* cublas_ref, int N)
{
    double matmul_time = std::numeric_limits<double>::max();

    matmul_time = std::min(matmul_time, check_configuration_async_BT_DB_FB_Idx<8, 8, 2, 4>(a, b, output, N));
    // debug_print(b,  N, true);
    // debug_print(output,  N, true);
    // debug_print(cublas_ref,  N, false);
    test_output(cublas_ref, N);

    matmul_time = std::min(matmul_time, check_configuration_async_BT_DB_FB_Idx<16, 8, 2, 4>(a, b, output, N));
    test_output(cublas_ref, N);


    matmul_time = std::min(matmul_time, check_configuration_async_BT_DB_FB_Idx<32, 8, 2, 4>(a, b, output, N));
    test_output(cublas_ref, N);
    return matmul_time;
}
