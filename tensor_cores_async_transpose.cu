...
__global__ void tensor_core_matmul_async_swizzle_BT_transpose(int n_elem, const half* a, const half* b, half* c)
{
    const int32_t warpM = (blockIdx.x*blockDim.x+threadIdx.x)/32;
    const int32_t warpN = blockIdx.y*blockDim.y+threadIdx.y;
    const int32_t laneM = threadIdx.x/32;
    ...
    for (int32_t tile = 0; tile < n_elem; tile+=BK*WMMA_MKN)
    {
        ...
        const half* b_curr = b + blockIdx.x*BM*WMMA_MKN*n_elem + tile;
        ...
        for (int i = (threadIdx.y * blockDim.x + threadIdx.x)*8;
            ...
            uint32_t b_smem_curr = __cvta_generic_to_shared(&b_smem[i^((i&(S_MASK<<S_BITS_A))>>S_BITS_A)]);
            const half* a_gmem_curr = &a_curr[(i/(WMMA_MKN*BK))*n_elem + i%(WMMA_MKN*BK)];
            CP_ASYNC_CG(b_smem_curr, reinterpret_cast<const float4*>(a_gmem_curr), 16);
        }
        CP_ASYNC_COMMIT_GROUP();
        CP_ASYNC_WAIT_GROUP(0);
        __syncthreads();
        for (int k = 0; k<BK && tile + k*WMMA_MKN < n_elem; k++)
        {
            ...
            for (int n = 0; n < OUT_TILES; n++)
            {
                ...
                load_tile_b_shared_swizzle_pre_transposed<S_BITS_B>(b_tile, b_smem, (laneM*OUT_TILES + n)*BK*WMMA_MKN*WMMA_MKN + k*WMMA_MKN, BK*WMMA_MKN, lane_id);
                for (int m = 0; m < OUT_TILES; m++)
                {
                    mma(a_tile[m], b_tile, acc[m][n]);
                }
...
template<int BM, int BN, int BK, int OUT_TILES>
double check_configuration_async_BT_transpose(half* a, half*b, half* output, half* transposed, int N)
{
    dim3 dimBlock(1,1,1);
    dim3 dimGrid(1,1,1);
    ...
    cudaFuncSetAttribute(tensor_core_matmul_async_swizzle_BT_transpose<BM, BN, BK, OUT_TILES>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);

    return measure_performance([&](){ 
            transposeCoalesced<<<dimGridT, dimBlockT>>>(transposed, b);
            tensor_core_matmul_async_swizzle_BT_transpose<BM, BN, BK, OUT_TILES><<<dimGrid, dimBlock, smem_size>>>(N, a, transposed, output);
            });
}
...
double TensorCoresAsyncTransposeKernel::run(half* a, half* b, half* cublas_ref, int N)
{
    ...
    matmul_time = std::min(matmul_time, check_configuration_async_BT_transpose<8, 8, 4, 4>(a, b, output, transposed, N));
    test_output(cublas_ref, N);
